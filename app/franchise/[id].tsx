  ...
  return (
    <SafeAreaView style={s.container} edges={['top', 'bottom']}>
      ...
      <ScrollView contentContainerStyle={{ paddingBottom: 24 }}>
        ...
        {own ? (
          <>
            ...
            <View style={s.btns}>
              ...
              <TouchableOpacity style={[s.btn, { borderColor: T.magenta }]} onPress={() => router.push({ pathname: '/create-movie', params: { brand: 'Sequel', franchiseId: franchise.id } })} testID="make-tv-series">
        ...
        <SectionHeader title="Franchise Entries & Sequels (Chronological)" />
        {(() => {
          const entries: Array<{
            id: string;
            actualId: string;
            title: string;
            type: 'movie' | 'series_season';
            brand: string;
            releaseYear: number;
            releaseWeek: number;
            score: number;
          ...
          movies.forEach(m => {
            entries.push({
              ...
              title: m.title,
              type: 'movie',
              brand: m.brand || 'Original',
              releaseYear: m.releaseYear || state.year,
              releaseWeek: m.releaseWeek || state.week,
              score: m.criticScore || 0,
              iconKey: m.iconKey,
              iconBg: m.iconBg,
              revenueLabel: `$${m.boxOffice.toFixed(2)}B Box Office`,
            });
          });

          // Add TV series seasons
          const linkedSeries = (state.tvSeries || []).filter(sr => sr.franchiseId === franchise.id);
          linkedSeries.forEach(sr => {
            sr.seasons.forEach(sn => {
              const snYear = sn.releaseYear ?? sr.releaseYear;
              const snWeek = sn.releaseWeek ?? sr.releaseWeek;
              if (sn.releaseWeek !== undefined || sr.status === 'released') {
                entries.push({
                  id: `${sr.id}-s${sn.number}`,
                  actualId: sr.id,
                  title: `${sr.title} (Season ${sn.number})`,
                  type: 'series_season',
                  brand: sr.brand ? sr.brand.toUpperCase() : 'ORIGINAL',
                  releaseYear: snYear,
                  releaseWeek: snWeek,
                  score: sn.avgScore || 0,
                  iconKey: 'television-play',
                  iconBg: T.magenta,
                  revenueLabel: `TV Series Season`,
                });
              } else {
                // Return current in-production season of TV series
                if (sn.number === sr.productionSeason) {
                  entries.push({
                    id: `${sr.id}-s${sn.number}`,
                    actualId: sr.id,
                    title: `${sr.title} (Season ${sn.number}) [Production]`,
                    type: 'series_season',
                    brand: 'IN PRODUCTION',
                    releaseYear: snYear,
                    releaseWeek: snWeek,
          ...
          entries.sort((a, b) => (a.releaseYear * 100 + a.releaseWeek) - (b.releaseYear * 100 + b.releaseWeek));

          if (entries.length === 0) {
            return <Text style={[s.sub, { padding: 12, fontStyle: 'italic' }]}>No entries released or in production yet.</Text>;
          }

          return entries.map((entry, idx) => {
            const isMovie = entry.type === 'movie';
            const detailPath = isMovie ? `/movie/${entry.actualId}` : `/series/${entry.actualId}`;
            return (
              <TouchableOpacity key={entry.id} style={s.movieRow} onPress={() => router.push(detailPath)} testID={`f-entry-${entry.id}`}>
                <IconTile icon={entry.iconKey} color={entry.iconBg} size={56} />
                <View style={{ flex: 1, paddingHorizontal: 10 }}>
                  <Text style={s.movieTitle}>{entry.title}</Text>
                  <Text style={s.sub}>
                    {isMovie ? '🎞 MOVIE' : '📡 TV SEASON'} #{idx + 1} · {entry.brand} · Y{entry.releaseYear} W{entry.releaseWeek} {entry.score > 0 ? `· critic: ${entry.score}/100` : ''}
                  </Text>
                </View>
                <Text style={[s.sub, { color: isMovie ? T.green : T.magenta, fontWeight: '800' }]}>
                  {entry.revenueLabel}
                </Text>
              </TouchableOpacity>
            );
