...
export default function Setup() {
  ...
  const start = async () => {
    await startNewGame(name.trim() || 'Horizon Studios', logoIdx);
  ...
  const startWithCash = async () => {
    const fresh = (await import('../src/game/sim')).newGame(name.trim() || 'Horizon Studios', logoIdx);
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={{ padding: 20 }}>
          ...
          <TextInput
            ...
            style={s.input} placeholder="e.g. Horizon Studios" placeholderTextColor={T.textMute}
...
const s = StyleSheet.create({
  ...
  label: { color: T.cyan, fontSize: 14, fontWeight: '800', marginTop: 16, marginBottom: 8, letterSpacing: 0.5 },
  ...
  tip: { color: T.textDim, fontSize: 12, marginTop: 16, textAlign: 'center', opacity: 0.8 },
});
