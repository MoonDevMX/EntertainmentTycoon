...
export default function CinemasScreen() {
  ...
  return (
    <SafeAreaView style={s.container} edges={['top', 'bottom']}>
      ...
      {/* Modern Cinema Performance Header Banner */}
      <View style={s.cinemaHeaderDeck}>
        <View style={s.headerInner}>
          <View style={[s.logoOuter, { backgroundColor: T.panelAlt }]}>
            <MaterialCommunityIcons name="movie-open" size={32} color={T.yellow} />
          </View>
          <View style={{ flex: 1, marginLeft: 14 }}>
            <Text style={s.brandSubtitle}>THEATRICAL & CINEMAS DESK</Text>
            <Text style={s.brandTitle}>Theatrical Distribution</Text>
            <Text style={s.ratingLabel}>Command theater circuits, negotiate split rates, and customize owned multiplexes</Text>
          </View>
        </View>
        ...
        <View style={s.glanceRow}>
          <View style={s.glanceCard}>
            <Text style={s.glanceLabel}>OWNED THEATERS</Text>
            <Text style={[s.glanceValue, { color: T.yellow }]}>{owned.length} Multiplexes</Text>
          </View>
          <View style={s.glanceCard}>
            <Text style={s.glanceLabel}>CIRCUIT AGREEMENTS</Text>
            <Text style={[s.glanceValue, { color: T.cyan }]}>{playerDeals.length} Chains</Text>
          </View>
          <View style={s.glanceCard}>
            <Text style={s.glanceLabel}>LIFETIME REVENUE</Text>
            <Text style={[s.glanceValue, { color: T.green }]}>
              ${state.player.totalBO.toFixed(2)}B
            </Text>
          </View>
        </View>
      </View>
                  ...
                  return (
                    <View key={p.id} style={s.proposalCard} testID={`cin-mgr-${p.id}`}>
                      ...
                      <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                        ...
                        <TouchableOpacity style={[s.actionBtn, { backgroundColor: T.red }]} onPress={() => { rejectCinemaOwnedManagerProposal(p.id); uiAlert('Dismissed', 'Suggestion was dismissed.'); }} testID={`cin-mgr-reject-${p.id}`}>
              ...
              return (
                <View key={p.id} style={s.proposalCard} testID={`proposal-${p.id}`}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                    ...
                    <TouchableOpacity style={[s.actionBtn, { backgroundColor: T.red }]} onPress={() => { rejectCinemaProposal(p.id); uiAlert('Rejected', 'Proposal was rejected.'); }} testID={`reject-${p.id}`}>
            ...
            {owned.length === 0 ? (
              ...
              <>
                <View style={{ flexDirection: 'row', gap: 6, marginVertical: 6, flexWrap: 'wrap' }}>
                  <TouchableOpacity
                    style={[s.licenseBtn, { flex: 1, minWidth: 140, backgroundColor: T.cyan }]}
                    onPress={openSchedOwned}
                    testID="open-sched-owned-btn"
                  >
                    <MaterialCommunityIcons name="calendar" size={16} color={T.cardDark} />
                    <Text style={s.licenseBtnTxt}>SCHEDULE RUN</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                ...
                {massMode && (
                  <View style={s.massBar} testID="mass-bar">
                    ...
                    <TouchableOpacity
                      style={[s.massActionBtn, { backgroundColor: T.cyan }, massSelected.size === 0 && { opacity: 0.4 }]}
                      onPress={() => {
                        if (massSelected.size === 0) return;
                        setSchedMovieIds(new Set());
                        setSchedCinemaIds(new Set(massSelected));
                        setSchedWeek(String(state.week));
                        setSchedYear(String(state.year));
                        setSchedWeeksToShow('6');
                        setSchedOwnedOpen(true);
                      ...
                      testID="mass-schedule-open"
                    >
                      <MaterialCommunityIcons name="calendar" size={14} color={T.cardDark} />
                      <Text style={s.massActionTxt}>Mass Schedule ({massSelected.size})</Text>
                    </TouchableOpacity>
                    <TouchableOpacity
              ...
              return (
                <View key={m.id} style={s.calMovieCard} testID={`cal-movie-${m.id}`}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                    ...
                    {owned.length > 0 ? (
                      <TouchableOpacity style={[s.smallBtn, { backgroundColor: ownedRuns.length > 0 ? T.yellow : T.green, marginLeft: 4 }]} onPress={() => {
                        setSchedMovieIds(new Set([m.id]));
                        setSchedCinemaIds(new Set(owned.map(oc => oc.id)));
                        setSchedWeek(String(state.week));
                        setSchedYear(String(state.year));
                        setSchedWeeksToShow('6');
                        setSchedOwnedOpen(true);
                      }} testID={`schedule-mine-${m.id}`}>
                        <Text style={[s.smallBtnTxt, { color: T.cardDark }]}>MINE</Text>
                      </TouchableOpacity>
                    ) : null}
                  </View>
                  {sched ? (
      ...
      <Modal visible={schedOwnedOpen} transparent animationType="slide" onRequestClose={() => setSchedOwnedOpen(false)}>
        <View style={s.modalBg}>
          <View style={s.modalCard}>
            ...
            {/* Quick multi-select controllers for region/size selection */}
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 4, marginVertical: 6 }}>
              <TouchableOpacity style={[s.smallBtn, { paddingHorizontal: 10, paddingVertical: 6, backgroundColor: T.cyan }]} onPress={() => setSchedCinemaIds(new Set(owned.map(c => c.id)))} testID="quick-sel-all">
                <Text style={{ fontSize: 10, fontWeight: '700', color: T.cardDark }}>All ({owned.length})</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[s.smallBtn, { paddingHorizontal: 10, paddingVertical: 6, backgroundColor: T.cardDark, borderWidth: 1, borderColor: T.border }]} onPress={() => setSchedCinemaIds(new Set())} testID="quick-sel-clear">
                <Text style={{ fontSize: 10, fontWeight: '700', color: T.textDim }}>Clear</Text>
              </TouchableOpacity>
              {['NA', 'EU', 'LATAM', 'ASIA', 'OCE', 'AFR'].map(reg => {
                const count = owned.filter(c => c.region === reg).length;
                if (count === 0) return null;
                return (
                  <TouchableOpacity key={reg} style={[s.smallBtn, { paddingHorizontal: 8, paddingVertical: 6, backgroundColor: T.yellow }]} onPress={() => setSchedCinemaIds(prev => {
                    const next = new Set(prev);
                    owned.filter(c => c.region === reg).forEach(c => next.add(c.id));
                    return next;
                  })} testID={`quick-sel-reg-${reg}`}>
                    <Text style={{ fontSize: 10, fontWeight: '700', color: T.cardDark }}>+{reg} ({count})</Text>
                  </TouchableOpacity>
                );
              })}
              {['small', 'medium', 'large', 'mega'].map(sz => {
                const count = owned.filter(c => c.size === sz).length;
                if (count === 0) return null;
                return (
                  <TouchableOpacity key={sz} style={[s.smallBtn, { paddingHorizontal: 8, paddingVertical: 6, backgroundColor: T.green }]} onPress={() => setSchedCinemaIds(prev => {
                    const next = new Set(prev);
                    owned.filter(c => c.size === sz).forEach(c => next.add(c.id));
                    return next;
                  })} testID={`quick-sel-sz-${sz}`}>
                    <Text style={{ fontSize: 10, fontWeight: '700', color: T.cardDark }}>+{sz.toUpperCase()} ({count})</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
            ...
            <View style={s.modalBg}>
              <ScrollView style={{ maxHeight: '92%' }}>
                <View style={s.modalCard}>
                  ...
                  <Text style={s.fieldLbl}>WEEKLY MARKETING BUDGET</Text>
                  <Text style={s.modalHint}>Active weekly marketing boosts ticket sales and local occupancy rates.</Text>
                  <View style={[s.custChipRow, { paddingBottom: 12 }]}>
                    {([0, 2, 5, 10, 20] as const).map(b => {
                      const active = (c.marketingBudgetM || 0) === b;
                      return (
                        <TouchableOpacity key={b} style={[s.custChip, active && { backgroundColor: T.green, borderColor: T.green }]}
                          onPress={() => { setEntityMarketing('cinema', c.id, b); }}
                          testID={`marketing-cinema-${c.id}-${b}`}>
                          <Text style={[s.custChipTxt, active && { color: T.cardDark }]}>{b === 0 ? 'OFF' : `$${b}M/wk`}</Text>
                        </TouchableOpacity>
                      );
                    })}
                  </View>
              ...
              return (
                <View key={p.id} style={s.ownedMgrCard} testID={`cin-mgr-${p.id}`}>
                  ...
                  <View style={{ flexDirection: 'row', gap: 8, marginTop: 10 }}>
                    ...
                    <TouchableOpacity style={[s.ownedMgrBtn, { backgroundColor: '#E84545' }]} onPress={() => { rejectCinemaOwnedManagerProposal(p.id); uiAlert('Dismissed', 'Suggestion was dismissed.'); }} testID={`cin-mgr-reject-${p.id}`}>
