...
export function tickOwnedCinemas(state: GameState): GameState {
  ...
  const updated: OwnedCinema[] = ownedCinemas.map(c => {
    ...
    const dueRun = (next.scheduledReleases || []).find(r => {
    ...
    if (next.currentMovieId) {
      ...
      if (movie) {
        ...
        myRevenue = +(slice * cinemaSideMult * amenityRevenueMult(c)).toFixed(4);
  ...
  let nextState = { ...state, ownedCinemas: updated, player, newsLog: [...news, ...state.newsLog].slice(0, 400) };
  // V43 — Ledger
...
export function ensureTVNetworks(state: GameState): GameState {
  ...
  const seeded: TVNetwork[] = TV_NETWORKS_SEED.filter(n => !existingIds.has(n.id)).map(n => ({
...
export function tickTVNetworks(state: GameState): GameState {
  ...
  if (!state.tvNetworkDeals || state.tvNetworkDeals.length === 0) {
    let s = { ...state, tvNetworks: networks, player };
  ...
  let s2 = { ...state, tvNetworkDeals: updatedDeals, franchises, streamingServices, tvNetworks: networks, player };
...
export function createTVSeries(state: GameState, args: {
  ...
  writerId?: string;
  directorId?: string;
  cast?: { talentId: string; role: 'lead_actor' | 'lead_actress' | 'support_actor' | 'support_actress' }[];
  ...
  // Enriched cast salaries (simplified: straight base salary for series, no BO % since no BO)
  const enrichedCast = (args.cast || []).map(c => {
    const t = state.talents.find(tt => tt.id === c.talentId);
    if (!t) return null;
    return { talentId: c.talentId, role: c.role, salary: +(t.salary * 0.6).toFixed(2) }; // TV pays ~60% of movie rate
...
export function tickTVSeries(state: GameState): GameState {
  ...
  const tvSeries = state.tvSeries.map(s => {
    ...
    const releasedSeasons = s.seasons.map(sn => sn.number === seasonNum ? { ...sn, releaseWeek: state.week, releaseYear: state.year, avgScore: 55 + Math.floor(Math.random() * 35) } : sn);
    ...
    if ((s.releaseStrategy === 'tv' || s.releaseStrategy === 'hybrid') && s.tvNetworkId) {
      ...
      if (net) {
        ...
        newsLog.push({ id: uid('news'), week: state.week, year: state.year, kind: 'license' as const, title: `${net.name} aired ${s.title} S${seasonNum}`, detail: `+$${feeB.toFixed(2)}B license fee.`, color: 'green' as const });
      }
    }
    ...
    newsLog.push({ id: uid('news'), week: state.week, year: state.year, kind: 'release' as const, title: `${s.title} — Season ${seasonNum} premiered`, detail: `${hybridLabel} · ${releasedSeasons[seasonNum - 1].avgScore}/100`, color: 'cyan' as const });
    ...
    if (fullyReleased) {
      ...
      if (involved.length > 0) {
        ...
        talents = talents.map(t => involved.includes(t.id) ? { ...t, inProductionMovieId: undefined, availableFromWeek: cdW, availableFromYear: cdY } : t);
      }
    }
    return {
      ...s,
  ...
  tvSeries.forEach(sr => {
    ...
    const inProd = sr.status === 'production' || (sr.productionWeeksLeft !== undefined && sr.productionWeeksLeft > 0);
...
export function tickPlayerCableNetworks(state: GameState): GameState {
  ...
  const newNets = list.map(net => {
    ...
    const newTiers = net.tiers.map(t => {
      const grow = t.subscribers * (priceFactor * contentFactor - 1);
...
export function tickChannelPacks(state: GameState): GameState {
  ...
  let next = { ...state, channelPacks, player };
...
export function proposeChannelContentLicense(state: GameState, args: { channelId: string; rivalStudioId: string; movieIds: string[]; askingFeeB: number; years: number }): { state: GameState; error?: string; licenseId?: string; counterFeeB?: number; accepted?: boolean } {
  ...
  if (args.askingFeeB >= minAccept) {
    ...
    const newsLog = [{ id: uid('news'), week: state.week, year: state.year, kind: 'license' as const, title: `${ch.name} licensed ${args.movieIds.length} film${args.movieIds.length !== 1 ? 's' : ''} from ${rival.name}`, detail: `$${args.askingFeeB.toFixed(2)}B × ${args.years}yr`, color: 'cyan' as const }, ...(state.newsLog || [])].slice(0, 400);
...
export function deleteTVChannel(state: GameState, channelId: string): { state: GameState; error?: string } {
  ...
  const newsLog = [{ id: uid('news'), week: state.week, year: state.year, kind: 'studio' as const, title: `${ch.name} shut down`, detail: `Channel closed. +$${refundB.toFixed(3)}B salvage.`, color: 'red' as const }, ...(state.newsLog || [])].slice(0, 400);
...
export function ensureCableProviders(state: GameState): GameState {
  if (state.cableProviders && state.cableProviders.length > 0) return state;
  return { ...state, cableProviders: CABLE_PROVIDERS_SEED.map(p => ({ ...p })) };
}
...
export function signCableCarriageDeal(state: GameState, args: { providerId: string; channelId: string; askingFeeUSD: number; years: number }): { state: GameState; error?: string; accepted?: boolean; counterFeeUSD?: number; dealId?: string } {
  ...
  if (args.askingFeeUSD <= q.maxUSD) {
    ...
    const newsLog = [{ id: uid('news'), week: state.week, year: state.year, kind: 'license' as const, title: `Carriage deal signed: ${prov.name} → ${ch.name}`, detail: `$${args.askingFeeUSD.toFixed(2)}/sub/mo × ${args.years}yr · est ~$${q.estWeeklyRevM.toFixed(2)}M/wk · +${boost.toFixed(1)}M subs`, color: 'green' as const }, ...(state.newsLog || [])].slice(0, 400);
...
export function tickCableCarriage(state: GameState): GameState {
  ...
  let next = { ...state, player, cableCarriageDeals };
...
export function tickAIStreamingLifecycle(state: GameState): GameState {
  ...
  for (const r of s.rivals) {
    ...
    if (myServices.length === 0 && myReleased.length >= 3 && r.cash >= 0.25 && Math.random() < 0.025) {
      const baseTiers = [
        { id: uid('t_'), name: 'Basic', monthlyPriceUSD: 7.99 },
        { id: uid('t_'), name: 'Standard', monthlyPriceUSD: 12.99 },
        { id: uid('t_'), name: 'Premium', monthlyPriceUSD: 17.99 },
      ];
...
export function tickTVNetworkManager(state: GameState): GameState {
  ...
  if (Math.random() < 0.22 && state.rivals.length > 0) {
...
export function tickCinemaOwnedManager(state: GameState): GameState {
  ...
  if (Math.random() > 0.45) return { ...state, cinemaOwnedManagerProposals: proposals };
...
export function signWholeNetworkLicenseOutbound(state: GameState, args: { rivalStudioId: string; movieIds: string[]; years: number; askingFeeB: number }): { state: GameState; error?: string; accepted?: boolean; counterFeeB?: number } {
  ...
  if (args.askingFeeB <= maxAccept) {
    ...
    let nextS = { ...state, player, rivals, tvNetworks, newsLog };
...
export function tickCinemaOwnedManagerExtended(state: GameState): GameState {
  const owned = state.ownedCinemas || [];
  if (owned.length === 0) return state;
  const player = state.player;
  const proposals = (state.cinemaOwnedManagerProposals || []).slice();
  // Auto-suggest scheduling: find an idle cinema + a recently released player movie not currently in that cinema.
  ...
  if (Math.random() > 0.35) return state;
  const recentMovies = state.movies.filter(m => m.studioId === player.id && m.status === 'released' && (state.year - m.releaseYear) <= 2 && m.boxOffice > 0.05);
  ...
  const newProposal: any = {
    ...
    rationale: `Idle cinema "${cinema.displayName || cinema.name}" could re-run "${movie.title}" (BO $${(movie.boxOffice * 1000).toFixed(0)}M) for 4 weeks. Estimated +$${(movie.boxOffice * 0.04 * 1000).toFixed(1)}M.`,
    createdWeek: state.week, createdYear: state.year,
  };
  const next = [...proposals, newProposal];
...
export function tickTVManagerOwnContent(state: GameState): GameState {
  const playerChannels = (state.tvNetworks || []).filter(n => n.ownerStudioId === state.player.id);
  if (playerChannels.length === 0) return state;
  const proposals = (state.tvManagerProposals || []).slice();
  if (proposals.length >= 6) return state;
  if (Math.random() > 0.20) return state;
